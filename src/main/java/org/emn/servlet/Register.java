...
public class Register extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (checkAttribute(request, "login")
				|| checkAttribute(request, "password")
				|| checkAttribute(request, "firstName")
				|| checkAttribute(request, "lastName")) {
			request.setAttribute("error", true);
			request.getRequestDispatcher(registerErrorPage).forward(request,
					response);
		} else {
			// Vérifie que les mots de passe correspondent
			if (request.getAttribute("password") != request
	...
	private Boolean checkAttribute(HttpServletRequest request, String attribute) {
		return request.getAttribute(attribute) == null
